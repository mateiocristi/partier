// import classes from "./style/UserProfile.module.css";
// import UpdateUserFrom from "../components/layout/UpdateUserForm";
// import {useEffect, useState} from "react";
// import AddNewCardForm from "../components/layout/AddNewCardForm";
// import M from "materialize-css";
// import options from "materialize-css";
// import OrderList from "../components/orders/OrderList";
// import FriendsList from "../components/friendsView/FriendsList";
// import Materialize from "materialize-css";
//
// const DEFAULT_IMG_URL = "data:image/jpeg;base64,";
// const MY_TICKETS = [{
//     orderId: 12,
//     eventName: "Untold",
//     eventId: 1,
//     orderDate: "12 july",
//     eventDate: "someDate"
// }, {orderId: 13, eventName: "Neversea", eventId: 2, orderDate: "someDate", eventDate:"14 august"}]
// const MY_FRIENDS = [{id: 1, name: "Matei", profilePicUrl: DEFAULT_IMG_URL}, {id: 2, name: "Matei", profilePicUrl: DEFAULT_IMG_URL}, {id: 3, name: "Matei", profilePicUrl: DEFAULT_IMG_URL}, {id: 4, name: "Matei", profilePicUrl: DEFAULT_IMG_URL}, {id: 5, name: "Matei", profilePicUrl: DEFAULT_IMG_URL}, {id: 6, name: "Matei", profilePicUrl: DEFAULT_IMG_URL}, {id: 7, name: "Matei", profilePicUrl: DEFAULT_IMG_URL}, {id: 8, name: "Matei", profilePicUrl: DEFAULT_IMG_URL}, {id: 9, name: "Matei", profilePicUrl: DEFAULT_IMG_URL}]
//
// function ClientUserProfilePage() {
//
//     const [selectedImage, setSelectedImage] = useState(undefined);
//     const [selectedImageURL, setSelectedImageURL] = useState("https://www.business2community.com/wp-content/uploads/2017/08/blank-profile-picture-973460_640.png");
//     const [currentImage, setCurrentImage] = useState();
//
//     initModal();
//     initTabs();
//
//     useEffect(() => {
//         // TODO: fetch friends
//         // fetch("/getProfilePic/" + JSON.parse(localStorage.getItem("user")).id)
//         //     .then(response => response.blob())
//         //     .then(imageBlob => {
//         //         console.log("img blob " + imageBlob);
//         //         console.log("url " + URL.createObjectURL(imageBlob));
//         //         // TODO: replace dummy image
//         //         setSelectedImageURL(URL.createObjectURL(imageBlob));
//         //
//         //     })
//         setSelectedImageURL(DEFAULT_IMG_URL);
//     }, [])
//
//     console.log("selected " + selectedImageURL);
//
//     function handleProfilePicChange(event) {
//         console.log(event.target.files[0]);
//         setSelectedImage(event.target.files[0]);
//         setSelectedImageURL(URL.createObjectURL(selectedImage));
//
//         const headers = new Headers();
//         headers.append("Content-Type", "multipart/form-data");
//
//         const formData = new FormData();
//         formData.append("file", selectedImage, JSON.parse(localStorage.getItem("user")).id);
//
//         const requestOptions = {
//             method: 'POST',
//             body: formData,
//             redirect: 'follow'
//         };
//
//         fetch("http://localhost:5000/api/uploadPicture", requestOptions)
//             .then(response => response.text())
//             .then(result => console.log(result))
//             .catch(error => console.log('error', error));
//
//     }
//
//     async function UpdateHandler(user) {
//         const headers = new Headers();
//         headers.append("Content-Type", "application/json");
//
//         const raw = JSON.stringify({
//             "address": user.address,
//             "firstName": user.firstName,
//             "lastName": user.lastName,
//             "password": user.password,
//             "email": user.email
//         });
//
//         const requestOptions = {
//             method: 'POST',
//             headers: headers,
//             body: raw,
//             redirect: 'follow'
//         };
//
//         fetch("http://localhost:5000/api/user/update/" + JSON.parse(localStorage.getItem("user")).id, requestOptions)
//             .then(response => response.text())
//             .then(result => console.log(result))
//             .catch(error => console.log('error', error));
//     }
//
//     async function AddCardHandler(cardDetails) {
//         const myHeaders = new Headers();
//         myHeaders.append("Content-Type", "application/json");
//
//         const raw = JSON.stringify({
//             "number": cardDetails.cardNumber,
//             "expYear": cardDetails.expYear,
//             "expMonth": cardDetails.expMonth,
//             "cvv": cardDetails.cvv
//         });
//
//         const requestOptions = {
//             method: 'POST',
//             headers: myHeaders,
//             body: raw,
//             redirect: 'follow'
//         };
//
//         fetch("http://localhost:5000/api/user/add-card/" + JSON.parse(localStorage.getItem("user")).username, requestOptions)
//             .then(response => response.text())
//             .then(result => {
//                 console.log("ok");
//                 if (result === "")
//                     Materialize.toast('Card added successfully', 3000, 'rounded')
//                 else {
//                     Materialize.toast('Invalid card details!', 3000, 'rounded')
//                 }
//             })
//             .catch(error => console.log('error', error));
//     }
//
//     function initModal() {
//         const elems = document.querySelectorAll('.modal');
//         M.Modal.init(elems, options, options);
//     }
//
//     function initTabs() {
//         const el = document.querySelector('.tabs');
//         const instance = M.Tabs.init(el, {}, options);
//     }
//
//     return (
//         <div className={classes.profileContainer}>
//             <div className={classes.imgContainer}>
//                 <img src={selectedImageURL}/>
//                 <div>
//                     <h1 className={classes.username}>{JSON.parse(localStorage.getItem("user")).username}</h1>
//                     <p>email: {JSON.parse(localStorage.getItem("user")).email}</p>
//                     <p>address: {"dummy address"}</p>
//                     {/*<p>name: {"John" + " " + "Tom"}</p>*/}
//                 </div>
//                 <ul className={classes.tabs + " tabs tabs-fixed-width"}>
//                     <li className="tab col s3"><a href="#friends" className="active">Friends</a></li>
//                     <li className="tab col s3"><a href="#test3">My Events</a></li>
//                     <li className="tab col s3"><a href="#tickets">My Tickets</a></li>
//                     <li className="tab col s3"><a href="#test4">Payment Methods</a></li>
//                 </ul>
//             </div>
//             <div id="tickets" className="col s12">
//                     <OrderList orders={MY_TICKETS}/>
//             </div>
//             <div id="friends" className="col s12">
//                 <FriendsList friends={MY_FRIENDS}/>
//             </div>
//             <div id="test3" className="col s12"></div>
//             <div id="test4" className="col s12"></div>
//
//
//             {/*<div className={classes.formContainer}>*/}
//             {/*    <form className={classes.form}>*/}
//             {/*        <input*/}
//             {/*            type="file"*/}
//             {/*            name="profilePic"*/}
//             {/*            onChange={handleProfilePicChange}*/}
//             {/*        />*/}
//             {/*    </form>*/}
//             {/*</div>*/}
//             <div className="fixed-action-btn">
//                 <a className="btn-floating btn-large red">
//                     <i className="large material-icons modal-trigger" data-target="modal1">mode_edit</i>
//                 </a>
//             </div>
//             <div id="modal1" className="modal modal-fixed-footer">
//                 <div className="modal-content">
//                     <p>Edit profile</p>
//                     <UpdateUserFrom onUpdateUser={UpdateHandler}/>
//                     <p>Add payment method</p>
//                     <AddNewCardForm onAddCardHandler={AddCardHandler}/>
//                 </div>
//                 <div className="modal-footer">
//                     <button className="modal-action modal-close btn-flat ">Confirm</button>
//                 </div>
//             </div>
//         </div>
//     )
// }
//
// export default ClientUserProfilePage;